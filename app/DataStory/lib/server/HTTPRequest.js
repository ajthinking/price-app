...
const Feature_1 = __importDefault(require("../../core/Feature"));
const NodeParameter_1 = __importDefault(require("../../core/NodeParameter"));
            ...
            try {
                for (var _b = __asyncValues(this.input()), _c; _c = yield _b.next(), !_c.done;) {
                    ...
                    yield this.request(feature)
                        .then((result) => {
                        this.output([new Feature_1.default(result)], 'Response');
                        if (this.getParameterValue('features_path')) {
                            ...
                            this.output(wrapped.map(r => new Feature_1.default(r)), 'Features');
                        }
                    }).catch((reason) => {
                        if (reason) {
                            this.output(
                            // Prevent functions in data
                            [new Feature_1.default(JSON.parse(JSON.stringify(reason)))], 'Failed');
                        }
                    });
                }
            }
